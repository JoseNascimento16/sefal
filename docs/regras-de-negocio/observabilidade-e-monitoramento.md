...
| 25/09/2026 | José Nascimento | Sistema (todas as telas) | O fuso da aplicação passa de UTC para **America/Bahia** (`config/app.php`, `APP_TIMEZONE`). Toda hora gravada e mostrada — trâmite, vistoria, log, exclusão de conta — e o agendador (limpeza de usuários às 03:00) passam a seguir o horário de Salvador. O que foi gravado antes ficou em UTC e aparece três horas adiantado; era dado de teste e de demonstração, sem correção retroativa. | Pedido do dono: as horas saíam três horas adiantadas em todas as telas. |
