...
**O que a linha CARREGA é outra regra, e ela tem documento próprio:** grade
enxuta, uma linha por registro com altura fixa, no máximo cinco colunas, texto
livre fora da célula, detalhe no clique e arquivo exportado completo. A régua, com
o porquê de cada item, está em **[`docs/padroes/listagem-clean.md`](../padroes/listagem-clean.md)**
— aplicada em `config/listagens_da_retaguarda.php` e travada por
`tests/Feature/ListagemCleanTest.php`. Não repita a régua aqui: um dia as duas
cópias divergiriam, e a que ninguém lê é a que fica errada.
...
|---|---|---|---|---|
| 25/09/2026 | José Nascimento | Casca | Menu: Monitoramento, Logs e Acompanhamento de Requisitos entram na pasta **Sistema**; **Operações** vira menu pai entre os mapas e o Sistema; Áreas e Equipes sai do menu (cadastros de Áreas e de Equipes). Cadastros e parametrização: os **botões do registro vêm em cima** do formulário, e em edição Voltar e Salvar se repetem no pé (`.rt-barra-registro`). | Pedidos do dono. |
