...
|---|---|---|---|---|
| 09/09/2026 | José Nascimento | Exportação de listagens | As colunas do arquivo de cada listagem passaram a ser declaradas no servidor (`config/listagens_da_retaguarda.php`), ao lado das colunas da grade. A grade encolheu por ordem do dono; **o arquivo não**, e agora isso é conferido por teste: tudo o que a tela mostra e tudo o que desceu para o detalhe tem de estar na exportação. | Enxugar a tela e apagar a coluna vizinha do `BotaoExportar` no mesmo impulso faria o arquivo lido por quem decide perder o dado **em silêncio** — ninguém reclama de uma coluna que nunca viu. Régua em [`docs/padroes/listagem-clean.md`](padroes/listagem-clean.md). |
