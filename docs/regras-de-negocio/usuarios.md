...
| 25/09/2026 | José Nascimento | Usuários | "Setores" passa a se chamar **Cargo** na tela (o SEFAL já é um setor). A conta ganha as duas marcas do Codecon — **Pode ativar o Modo Gerente** e **Administrador de usuários** (`users.is_gerente`, `users.is_admin_usuarios`) —, que dão, cada uma, o pacote inteiro de UMA tela por cima do cargo; só o administrador as dá ou tira. | Pedido do dono. |
