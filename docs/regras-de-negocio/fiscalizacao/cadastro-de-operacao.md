...
**Onde fica:** Menu → **Sistema** → Cadastro de Operação (`/retaguarda/operacoes`).
**Quem usa:** o líder de equipe (cadastra as da área da equipe dele), o Chefe de Setor (cadastra em
qualquer área) e o administrador. **O fiscal não entra** (ver RN-07).
...
|---|---|---|---|---|
| 09/09/2026 | José Nascimento | Cadastro de Operação | **A grade ficou enxuta** (padrão [`docs/padroes/listagem-clean.md`](../../padroes/listagem-clean.md)): **Operação · Área · Período · Equipes · Situação**. O **foco** (texto livre) saiu de baixo do nome, a chefia saiu de baixo da área e o alcance (região + contagem de bairros) saiu da célula — os três estão na ficha da operação e no arquivo exportado. As equipes deixaram de ser uma fileira de chips (que quebrava com quatro equipes) e viraram texto: o selo da linha é a situação. | Ordem do dono (09/09/2026): _"as listagens estão muito poluídas, muita informação quebrando linha de forma irregular… deixe a informação detalhada para quando o usuário clicar"_. A régua e o porquê de cada item ficam em [`docs/padroes/listagem-clean.md`](../../padroes/listagem-clean.md) — este doc aponta para lá em vez de repetir a régua. |
...
| 10/09/2026 | José Nascimento | Cadastro de Operação | A tela **mudou de lugar no menu**: saiu da seção "Fiscalização" e passou a ser item do menu de **Sistema**. Rota, `slug` (`operacoes`), concessão e regras ficaram **intactos**. | Ordem do dono (10/09/2026): _"crie no menu 'Sistemas' e coloque como filhos: Áreas e Equipes; Cadastro de Operação"_. Montar operação é ato de gestão sobre a estrutura, e não trabalho de campo — fica junto de Áreas e Equipes, que é a estrutura em cima da qual ela é montada. |
| 22/09/2026 | José Nascimento | Cadastro de Operação | Quem cadastra: o **líder de equipe**, recortado pela **área da(s) equipe(s) dele** (`Papel::areas`, props `areasDoLider`/`recorteDeArea`/`lideres`), e o **Chefe de Setor**, em qualquer área — ele recebe a operação pedida de cima (a "avulsa") e a monta para a equipe. Não existe mais "quem só consulta": a concessão do líder no menu deixou de ser apenas-leitura. O cartão mostra o **líder** da equipe da operação no lugar do "chefe da área". | Reforma dos papéis de 22/09/2026 — ver [Papéis e setores](../papeis-e-setores.md). |
| 25/09/2026 | José Nascimento | Cadastro de Operação | Vira **menu pai** próprio, entre os mapas e o Sistema. A operação recebe **denúncias** no cadastro e na edição (pela mesma porta do "Incluir em operação": Em operação, com a equipe da operação; desmarcada, volta à mesa de onde veio) e **fiscais de qualquer área** (`operacao_fiscais`), que recebem a fiscalização junto com os das equipes (`Operacao::fiscaisQueRecebem`). Escolher a área ou uma equipe **marca os bairros** dela, desmarcáveis um a um. Botões também em cima. | Pedidos do dono. |
| 25/09/2026 | José Nascimento | Cadastro de Operação | Desmarcar uma equipe (ou trocar a área) **tira os bairros** dela — menos os que a área escolhida ou outra equipe marcada ainda cobre. | Retorno do dono. |
