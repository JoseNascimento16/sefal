...
|---|---|---|---|---|
| 04/09/2026 | José Nascimento | Denúncias (trâmite) | **O catálogo de recomendações do fiscal passa a ser mapa `chave => {curto, explicito}`, e o que se grava é a CHAVE** (RN-17b). Os atalhos semeados no trâmite deixam de ser a frase inteira e passam a ser chave; a leitura (trâmite e [Retorno de Campo](retorno-de-campo.md)) traduz pela redação **explícita**, que vem do servidor, e chave desconhecida aparece **crua** em vez de desaparecer. A lista unificada ganhou `seab`, `guarda`, `passagem`, `reprogramar`, `chefia` e `outro-orgao`, e `sgci` passou a dizer "Conferir o cadastro do ambulante **no SGCI**" (antes "na Retaguarda"). | O catálogo existia em duas versões incompatíveis — a Retaguarda esperava a frase inteira e o aplicativo do fiscal gravava chave —, então o despacho do fiscal chegaria aqui como recomendação que a tela não sabe ler e o relatório não sabe somar. Decisão do dono: unificar com a redação curta no aplicativo e a explícita na Retaguarda. |
